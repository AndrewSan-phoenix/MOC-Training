...
<section class="relative py-16 bg-gradient-to-r from-blue-800 to-blue-600">
    ...
    <div class="container mx-auto px-4 relative z-10">
        <div class="text-center text-white max-w-3xl mx-auto">
            ...
            <p class="text-lg md:text-xl text-blue-100">Explore information about our students, their courses, and batches.</p>
        </div>
    </div>
</section>
    ...
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="flex justify-center items-center my-8">
            <div class="relative w-full max-w-2xl">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-purple-500">
                ...
                <div id="dropdownMenu" class="absolute left-0 right-0 top-14 bg-white border border-purple-200 rounded-xl shadow-lg z-20 max-h-60 overflow-auto hidden">
                    <div class="py-2">
                        <div class="px-6 py-2 hover:bg-purple-50 cursor-pointer batch-option" data-value="all">All Batches</div>
                        {{-- Loop through all batches to populate dropdown --}}
        ...
        <div class="overflow-x-auto bg-white rounded-lg shadow-md">
            <table class="min-w-full border-collapse border border-gray-400 divide-y divide-gray-200" id="studentsTable">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="studentsTableBody">
                    {{-- Student rows will be loaded here by JavaScript --}}
                </tbody>
            </table>
        </div>

        {{-- Pagination controls --}}
        <div class="flex items-center justify-between px-4 py-3 sm:px-6 mt-8">
            {{-- Mobile pagination --}}
...
<div id="imageModal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 hidden">
    <div class="relative bg-white p-4 rounded-lg shadow-lg max-w-xl max-h-full overflow-auto">
        ...
        <img id="zoomedImage" src="" alt="Zoomed Student Image" class="max-w-full max-h-[80vh] object-contain mx-auto">
        <p id="modalStudentName" class="text-center mt-2 text-lg font-semibold text-gray-800"></p>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const modalStudentName = document.getElementById('modalStudentName'); // Changed from modalTeacherName
    const closeModalBtn = document.getElementById('closeModal');

    let currentPage = 1;
    const studentsPerPage = 6; // Number of students to display per page (matching teacher's page design)
    let totalPages = 1;
    let totalStudents = 0; // To store the total number of students for pagination info
    ...
    function renderPagination() {
        ...
        const maxPagesToShow = 6; // Adjust as needed
        ...
        const createPageButton = (pageNumber, isActive) => {
            ...
            if (isActive) {
                ...
                pageBtn.addEventListener('click', () => {
                    ...
                    fetchStudents(); // Call fetchStudents instead of fetchTeachers
                });
            }
        ...
        const fromEntry = (currentPage - 1) * studentsPerPage + 1;
        const toEntry = Math.min(currentPage * studentsPerPage, totalStudents);
        fromEntrySpan.textContent = totalStudents === 0 ? 0 : fromEntry;
        toEntrySpan.textContent = toEntry;
        totalEntriesSpan.textContent = totalStudents;

        // Update "Showing X to Y of Z results" for Mobile
        fromEntryMobileSpan.textContent = totalStudents === 0 ? 0 : fromEntry;
        toEntryMobileSpan.textContent = toEntry;
        totalEntriesMobileSpan.textContent = totalStudents;
    ...
    /**
     * Fetches students data from the server via AJAX and updates the table.
     */
    function fetchStudents() {
        console.log('Fetching students for page:', currentPage, 'search:', currentSearchValue, 'batch_filter:', currentBatchFilter);

        studentsTableBody.innerHTML = `<tr><td colspan="6" class="text-center py-4 text-gray-500">Loading students...</td></tr>`;
        noResultsMessage.classList.add('hidden');

        $.ajax({
            url: "{{ route('get.students.ajax') }}", // Ensure this route is correct for students
            method: 'GET',
            data: {
                page: currentPage,
                search: currentSearchValue,
                batch_filter: currentBatchFilter,
                students_per_page: studentsPerPage // Pass studentsPerPage to the backend
            },
            success: function(response) {
                ...
                if (response.data && response.data.length > 0) {
                    response.data.forEach(student => {
                        ...
                        row.dataset.batches = student.batches_data || '';

                        // Fallback for profile image
                        const profileImageUrl = student.profile_image ? student.profile_image : `{{ asset('images/default-profile.png') }}`;
                        ...
                        row.innerHTML = `
                            <td class="border border-gray-300 px-6 py-4 whitespace-nowrap">
                                <div class="flex-shrink-0 w-12 h-12 rounded-full overflow-hidden border border-gray-200 cursor-pointer image-thumbnail">
                                    <img class="object-cover w-full h-full" src="${profileImageUrl}" alt="${student.name}" data-full-image="${profileImageUrl}" data-student-name="${student.name}">
                                </div>
                            </td>
                            ...
                            <td class="border border-gray-300 px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                ${student.batches_data ? student.batches_data.split(';').map(batch => `<div>${batch}</div>`).join('') : 'No Enrollments'}
                            </td>
                            <td class="border border-gray-300 px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                ${student.phone || 'N/A'}
                            </td>
                ...
                if (!resultsFound) {
                    studentsTableBody.innerHTML = `<tr><td colspan="6" class="border border-gray-300 px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">No students found matching your criteria.</td></tr>`;
            ...
            error: function(xhr, status, error) {
                ...
                studentsTableBody.innerHTML = `<tr><td colspan="6" class="border border-gray-300 px-6 py-4 whitespace-nowrap text-center text-sm text-red-500">Error loading students. Please try again.</td></tr>`;
                noResultsMessage.classList.remove('hidden');
            }
        });
    }
    ...
    // Mobile Pagination Event Listeners (identical to desktop now)
    ...
    combinedInput.addEventListener('input', () => {
        ...
        debounceTimer = setTimeout(() => {
            ...
            if (newSearchValue !== currentSearchValue) {
                ...
                document.querySelector('.batch-option[data-value="all"]').classList.add('selected'); // Highlight "All Batches"
...
<style>
    ...
    input:focus {
        border-color: #6b46c1; /* Purple-500 equivalent */
    ...
    .batch-option.selected {
        ...
        font-weight: 600;
        color: #6b46c1; /* Purple-700 equivalent */
    }
